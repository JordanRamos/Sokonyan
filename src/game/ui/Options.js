...
function (
	...
	Options.prototype.init = function (UIManager) {
		...
		$("#Options").append("<div class='buttonCloseOptions'>" + txt.get("LABEL_POPUP_CLOSEBTN") + "</div>");
		$("#Options .buttonCloseOptions").css("background-image", "url(" + SpriteManager.get("buttonLoginStatic").src + ")");

		// Hover
		$( "#Options .buttonCloseOptions").hover(function() {
			$( this ).css("background-image", "url(" + SpriteManager.get("buttonLoginSurvol").src + ")");
			$("#Options .buttonCloseOptions").css("background-repeat", "no-repeat");
		...
		// Active
		$("#Options .buttonCloseOptions").mousedown(function() {
			$("#Options .buttonCloseOptions").css("background-image", "url(" + SpriteManager.get("buttonLoginPress").src + ")");
			$("#Options .buttonCloseOptions").css("padding-top", 12);
		});

		$("#Options .buttonCloseOptions").mouseup(function() {
			$("#Options .buttonCloseOptions").css("background-image", "url(" + SpriteManager.get("buttonLoginStatic").src + ")");
			$("#Options .buttonCloseOptions").css("padding-top", 8);
			$("#blackScreen").hide();
